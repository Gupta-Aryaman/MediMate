...
export default function DashboardPage() {
    ...
    useEffect(() => {
        // Scroll to the last message after updating the chat
        const lastMessage = document.querySelector('.flex-grow > div:last-child');
        if (lastMessage) {
    ...
    useEffect(() => {
        // Call handleClearHistory when the component mounts (user logs in)
        handleClearHistory();
    }, []);

    useEffect(() => {
        // Clear the chat log when clearChat state changes
        if (clearChat) {
            setChatLog([]);
            // Reset the clearChat state to false
            setClearChat(false);
        }
    }, [clearChat]);
        ...
        <div>
            ...
            <div className="container mx-auto max-w-[1000px] py-7" >
                ...
                <form onSubmit={handleSubmit} className="flex-none p-6">
                    ...
                    <input type="text" placeholder="Type Your message" onChange={(e) => setInputValue(e.target.value)} value={inputValue} className="flex-grow px-4 py-2 bg-transparent text-white focus:outline-none" />
